...
import events from 'closure-events';
...
export class Tab extends events.EventTarget {
    constructor(head, path){
        ...
        // Tab body
        this.body = q.id(head.getAttribute('id').replace('tab', 'tab-body-'));
...
function _processAjaxLink(link, isActive, callback){
    ...
    Gateway.get(link, null, function(data){
        ...
        _.each(this.root.qAll('form'), function(form){
...
function _initAjaxLinks(body){
    ...
    _.each(body.qAll(LINK_SELECTOR), function(link, index){
...
function _showLoading(element){
    var loader = element.q('.tab-loader');
    if(_.isNull(loader)){
